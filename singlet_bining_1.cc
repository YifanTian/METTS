...
for (int i = 1;i < 1000;i++){
  ...
  cout<<i<<" "<<get_rand<<endl;
}
*/

//for (int ent = 78; ent <= 78; ent=ent+10) {
...
for(int tstep = 1; tstep <= thermal_step; tstep++)        //thermal_step
  {
    ...
    if (cps[N] == 1) {
      ...
      outfile_config<<tstep<<" "<<spinz<<endl;
        ...
        for(int n = 1; n < N; n = n+2)
         {
          ...
          //cout<<tstep<<" rand: "<<rand()%(10000-0+1)<<" "<<get_rand<<endl;
          //cout<<tstep<<" rand: "<<get_rand<<endl;
          //PrintData(wf);
    ...
    if (tstep>=heatstep)
    {
      ...
      //cout<<tstep<<" "<<int(((tstep+50)/100)-1)<<endl;
